...
def main():
    ...
    for k, test_index in enumerate(kf):
        ...
        dataset = dataset.cache().repeat().shuffle(len(train_index), reshuffle_each_iteration=True).batch(BATCH_SIZE).prefetch(tf.data.experimental.AUTOTUNE)
        ...
        for i in range(0,len(test_index)):
            pred = np.squeeze(model.predict(test_noisy[i:i+1,:,:,np.newaxis])) # noisy: [1, H, W, 1] or [1, H, W, C]
            psnr_tot += psnr(test_label[i], pred, data_range=255.0)
            ssim_tot += ssim(test_label[i], pred, data_range=255.0)
        print("%d-fold cross validation PSNR: %f, SSIM %f" % (k, psnr_tot/len(test_index), ssim_tot/len(test_index)))
